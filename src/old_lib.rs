...
use syn::{parse_macro_input, DataStruct, Data};
#[macro_use]
extern crate quote;
...
fn impl_with_generator<G: BodyGenerator>(ast: &syn::DeriveInput, gen: G) -> TokenStream {
...
trait BodyGenerator {
    ...
    fn quote_borrowed_params(&self, ast: &syn::DeriveInput) -> Vec<TokenStream> {
    ...
    fn quote_type_params(&self, ast: &syn::DeriveInput) -> Vec<TokenStream> {
    ...
    fn quote_rhs_params(&self, ast: &syn::DeriveInput) -> Vec<TokenStream> {
    ...
    fn visit_struct(&self, data: &Data) -> TokenStream;
    fn visit_enum_data(&self, variant: TokenStream, data: &Data) -> TokenStream;
    fn combine_impl(&self, borrows: TokenStream, name: &syn::Ident, rhs_params: TokenStream, owned: TokenStream, body: TokenStream) -> TokenStream;
}
...
impl BodyGenerator for IntoOwnedGen {
    ...
    fn visit_enum_data(&self, ident: TokenStream, data: &Data) -> TokenStream {
        match *data {
            Data::Struct(ref data) => {
                ...
                let idents = data.iter()
                    .map(|field| field.ident.as_ref().unwrap())
                    .collect::<Vec<_>>();
                let cloned = data.iter()
                    .map(|field| {
    ...
    fn combine_impl(&self, borrowed: TokenStream, name: &syn::Ident, params: TokenStream, owned: TokenStream, body: TokenStream) -> TokenStream {
        quote! {
...
impl FieldKind {
    ...
    fn resolve(field: &syn::Field) -> Self {
        match &field.ty {
            &syn::Type::Path(syn::Path { ref segments, .. }) => {
                if is_cow(segments) {
                    FieldKind::PlainCow
    ...
    fn move_or_clone_field(&self, var: &TokenStream) -> TokenStream {
        use self::FieldKind::*;
    ...
    fn borrow_or_clone(&self, var: &TokenStream) -> TokenStream {
        use self::FieldKind::*;
...
fn has_lifetime_arguments(segments: &[syn::PathSegment]) -> bool {
    if let Some(&syn::PathArguments::AngleBracketed(ref args)) = segments.last().map(|x| &x.arguments) {
        !args.args.into_iter().any(|f| matches!(f, syn::GenericArgument::Lifetime(_)))
    } else {
        false
    }
}
...
fn number_of_type_arguments(segments: &[syn::PathSegment]) -> usize {
    if let Some(&syn::PathArguments::AngleBracketed(ref args)) = segments.last().map(|x| &x.arguments) {
        args.args.into_iter().filter(|f| matches!(f, syn::GenericArgument::Type(_))).count()
    } else {
        0
    }
}
...
fn has_binding_arguments(segments: &[syn::PathSegment]) -> bool {
    if let Some(&syn::PathArguments::AngleBracketed(ref args)) = segments.last().map(|x| &x.arguments) {
        !args.args.into_iter().any(|f| matches!(f, syn::GenericArgument::Binding(_)))
    } else {
        false
    }
}
    ...
    loop {
        if type_hopefully_is(segments, "std::option::Option") {
            match *segments.last().unwrap() {
                syn::PathSegment { arguments: syn::PathArguments::AngleBracketed(ref data), .. } => {
                    ...
                    match *data.args.first().unwrap() {
                        syn::Type::Path(None, syn::Path { segments: ref next_segments, ..}) => {
                            levels += 1;
...
fn is_iter_field(mut segments: &Vec<syn::PathSegment>) -> Option<FieldKind> {
    loop {
        ...
        if type_hopefully_is(segments, "std::vec::Vec") {
            match *segments.last().unwrap() {
                syn::PathSegment { parameters: syn::PathArguments::AngleBracketed(ref data), .. } => {
                    if !data.lifetimes.is_empty() || !data.bindings.is_empty() {
                        break;
                    }
